...
for a in range(1, 49):
    for b in range(1, 11):
        location = '../PSPLIB_dataset/j90.sm/j90{}_{}.sm'.format(a, b)
        f = open(location, 'r')
        data = f.readlines()

        information = []
        adj_mat = np.zeros(8464).reshape(92, 92)
        resource_mat = np.zeros(644).reshape(92, 7)
        resource_capacity_mat = np.zeros(4)
        # 读取邻接矩阵 #
        for line_num in range(18, 110):
...
# 保存文件
path = '../PSPLIB_dataset/problems_90.npy'
