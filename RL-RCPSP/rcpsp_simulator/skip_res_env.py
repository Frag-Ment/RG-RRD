...
from rcpsp_simulator.skip_res_executor import Skip_res_executor
...
class Skip_res_environment(object):
    def __init__(self):
        self.executor = None
    ...
    def reset(self, adj_mat, nodes_information, resource, resource_variant):
        ...
        executor = Skip_res_executor(jobdag, resource_variant)
    ...
    def return_last_state(self, action):
        ...
        if self.executor.complete_node + len(self.executor.running_tasks) == self.executor.jobdag.num_nodes - 1:
            ...
            return self.tep(self.adj_mat, self.executor.feature_mat, self.executor.resource_exec,
                            self.executor.runable_nodes_idx), self.executor.walltime - self.last_decision_time, 1
