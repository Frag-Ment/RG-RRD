...
writer = SummaryWriter(f"runs/{args.use_dataset}_{time.time()}_normal")
writer.add_text(
    "hyperparameters",
...
if args.use_dataset == 'set_30':
    dataloader_train = Dataloader(train_set_30, repeat_times=repeat_times)
    dataloader_val = Dataloader(val_set_30)
    dataloader_test = Dataloader(test_set_30)
elif args.use_dataset == 'set_60':
    dataloader_train = Dataloader(train_set_60, repeat_times=repeat_times)
    dataloader_val = Dataloader(val_set_60)
    dataloader_test = Dataloader(test_set_60)
elif args.use_dataset == 'set_90':
    dataloader_train = Dataloader(train_set_90, repeat_times=repeat_times)
    dataloader_val = Dataloader(val_set_90)
    dataloader_test = Dataloader(test_set_90)
elif args.use_dataset == 'set_120':
    dataloader_train = Dataloader(train_set_120, repeat_times=repeat_times)
    dataloader_val = Dataloader(val_set_120)
    dataloader_test = Dataloader(test_set_120)
elif args.use_dataset == 'set_mix':
    dataloader_train = Dataloader(train_set_mix, repeat_times=repeat_times)
    dataloader_val = Dataloader(val_set_mix)
    dataloader_test = Dataloader(test_set_mix)
...
agent = Agent(num_layers=args.gnn_layer, learn_eps=False, num_mlp_layers_feature_extract=args.fea_extract_layer,
        ...
        num_mlp_layers_critic=3, hidden_dim_critic=32,
...
for update in tqdm(range(1, num_updates + 1)):
    ...
    if update % 20 == 0:
        print('saved')
        save_model_name = 'saves/' + args.use_dataset + str(update) + '_'+\
                          str(args.gnn_layer)+ str(args.fea_extract_layer)+str(time.time()) +'.pt'
