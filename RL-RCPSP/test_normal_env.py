...
writer = SummaryWriter("runs/test_repeat1_normal_env_vf1")
writer.add_text(
    "hyperparameters",
...
# ini dataset
train_set_30 = [['30', 200, 320, 'None']]
val_set_30 = [['30', 0, 90, 'None']]
test_set_30 = [['30', 200, 280, 'None']]
...
for update in tqdm(range(1, num_updates + 1)):
    ...
    if update % 5 == 0:
        print('saved')
        save_model_name = 'saves/' + args.use_dataset + '_'+ 'normalenv'+ '_' + str(update) + '.pt'
        torch.save(agent.state_dict(), save_model_name)
        # compute average makespan
        train_makespan = greedy_average_makespan(agent, dataloader_train, normal_env=True)
        val_makespan = greedy_average_makespan(agent, dataloader_val, normal_env=True)
